...
def select_match(player_1, mode, player_2=None, game_type =None, stage=None):
    ...
    if mode == 'smash':
        if game_type and stage:
            start_smash_output = start_smash(stage,game_type)
            return return_strs + start_smash_output
        elif stage:
            start_smash_output = start_smash(stage = stage)
            return return_strs + (start_smash_output,)
        elif game_type:
            start_smash_output = start_smash(game_type = game_type)
            return return_strs + (start_smash_output,)
        else:
            start_smash()
            return return_strs
        ...
        if game_type:
            start_classic_output = start_classic(game_type)
            return return_strs + (start_classic_output,)
        else:
            start_classic()
            return return_strs
...
def main():
    ...
    """
    match1 = start_smash(stage = "Mushroomy Kingdom")
    ...
    smash_bros_matches = [select_match(player_1 = "Yoshi", mode = "smash", stage = "Paper Mario")]
